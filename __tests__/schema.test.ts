...
const HL: Highlight[] = [
  {
    highlightId: "H1",
    text: "Small changes compound over time to produce remarkable results.",
  },
  {
    highlightId: "H2",
    text: "The four laws: make it obvious, attractive, easy, satisfying.",
  },
  { highlightId: "H3", text: "Environment design beats willpower." },
  {
    highlightId: "H4",
    text: "Two-Minute Rule: scale any habit to two minutes.",
  },
  {
    highlightId: "H5",
    text: "Habit stacking links new habits to existing routines.",
  },
  { highlightId: "H6", text: "Identity-based habits: become the person." },
  {
    highlightId: "H7",
    text: "Plateau of Latent Potential: results after threshold.",
  },
...
describe("normalizeNotes", () => {
  it("splits paragraphs", () => {
    // Each chunk must be >=3 chars to pass the normalizer filter
    expect(
      normalizeNotes(
...
describe("interview", () => {
  it("passes 5 bullets ≤18 words", () => {
    expect(
      validateOutput({
        ...
        items: [
          {
            text: "Compounding yields massive long-term gains.",
            citations: ["H1"],
          },
          {
            text: "Four laws framework guides behavior change.",
            citations: ["H2"],
          },
          {
            text: "Environment beats willpower consistently.",
            citations: ["H3"],
          },
          {
            text: "Two-Minute Rule lowers activation barriers.",
            citations: ["H4"],
          },
  ...
  it("fails bullet >18 words", () => {
    // This bullet has 22 words — clearly over the 18-word limit
    expect(
      validateOutput({
        mode: "interview",
        items: [
          {
            text: "This particular bullet point has been written to contain significantly more than eighteen words to ensure it clearly exceeds the defined limit.",
            citations: ["H1"],
          },
          { text: "Short bullet.", citations: ["H2"] },
          { text: "Short bullet.", citations: ["H3"] },
          { text: "Short bullet.", citations: ["H4"] },
          { text: "Short bullet.", citations: ["H5"] },
        ],
      }).success,
    ).toBe(false);
  });
  ...
  it("passes with analogy", () => {
    expect(
      validateOutput({
        ...
        items: [
          {
            text: "Imagine habits are like building blocks.",
            citations: ["H1"],
          },
  ...
  it("fails >120 words", () => {
    expect(
      validateOutput({
        ...
        items: [
          {
            text: "Imagine " + Array(125).fill("word").join(" "),
            citations: ["H1"],
          },
